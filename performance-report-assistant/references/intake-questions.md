...
- 如果提供了代码仓库：默认先做研发数据统计，用提交次数、涉及文件、代码增删行、高频文件、作者分布、日期分布等角度支撑报告。
- 如果提供了代码仓库但没有指定分支：默认按时间范围统计全部分支，避免漏掉 feature/release/hotfix 分支。
- 如果用户明确指定分支：只统计该分支。
- 如果要统计“你的提交”：你的 git author 名称或邮箱是什么？如果不提供，我会先统计全部作者并在结果里注明没有按作者过滤。
...
```text
你提供了代码仓库。因为你没有指定分支，我会默认按时间范围统计全部分支，避免遗漏 feature/release/hotfix 分支上的提交。如果你只想看某个分支，请告诉我分支名。
```

```text
如果只统计你的提交，请告诉我 git author 名称或邮箱；如果不提供，我会先统计全部作者，并在结果里注明没有按作者过滤。
```
