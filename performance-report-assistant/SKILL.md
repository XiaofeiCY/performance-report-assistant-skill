...
- Treat copying as a verified current-result-page operation. The copy button may be below the visible area, in a fixed bottom action bar, or behind long scrollable content. After confirming the current result page, the collector uses template matching, regional OCR, lower combined bottom-of-window OCR/action-row geometry, and bounded in-page scrolling (max 6 passes). It must not use right-click, fixed-coordinate copy guesses, or Ctrl+A/Ctrl+C on unknown regions as a default fallback.
